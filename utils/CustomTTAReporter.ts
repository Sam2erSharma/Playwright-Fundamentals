...
<body><p>Redirecting to <a href="${path.basename(this.outputFile)}">latest report</a>...</p></body></html>`;
        ...
        this.generateReadmeReport(reportDir);
    }
...
<body>
    ...
    private generateReadmeReport(reportDir: string): void {
        try {
            ...
            let md = `# 🎭 TTA Automation Test Run Report\n\n`;
            md += `This README was automatically generated by **CustomTTAReporter** at the end of the test run.\n\n`;

            md += `## 📊 Run Summary\n\n`;
            md += `| Metric | Value |\n`;
            md += `| :--- | :--- |\n`;
            md += `| **📅 Run Date** | ${this.formatTime(this.startTime)} |\n`;
            md += `| **⏱️ Total Duration** | ${this.formatDuration(totalDuration)} |\n`;
            md += `| **🌐 Environment** | \\\`${env.toUpperCase()}\\\` |\n`;
            md += `| **🌍 Browser** | \\\`${browserName}\\\` |\n`;
            md += `| **💻 Platform** | ${platform} |\n`;
            md += `| **📈 Pass Rate** | **${passRate}%** |\n\n`;

            md += `## 📈 Status Breakdown\n\n`;
            md += `| Status | Count | Icon |\n`;
            md += `| :--- | :--- | :---: |\n`;
            md += `| **Passed** | ${this.suiteStats.passed} | ✅ |\n`;
            md += `| **Failed** | ${this.suiteStats.failed} | ❌ |\n`;
            md += `| **Skipped** | ${this.suiteStats.skipped} | ⏭️ |\n`;
            md += `| **Total** | ${this.suiteStats.total} | 📊 |\n\n`;

            md += `## 📋 Detailed Test Results\n\n`;
            md += `| S.No | Test Name | File | Duration | Status |\n`;
            md += `| :---: | :--- | :--- | :---: | :---: |\n`;

            let index = 1;
            for (const test of this.testResults) {
                const statusIcon = test.status === 'passed' ? '✅' : test.status === 'failed' || test.status === 'timedOut' ? '❌' : '⏭️';
                const durationStr = this.formatDuration(test.duration);
                const fileName = test.file.split(/[/\\]/).pop() || test.file;
                md += `| ${index} | **${test.title}** | \\\`${fileName}\\\` | ${durationStr} | ${statusIcon} ${test.status.toUpperCase()} |\n`;
                index++;
            }
            md += `\n`;

            // If there are failures, list them with details
            const failedTests = this.testResults.filter(t => t.status === 'failed' || t.status === 'timedOut');
            if (failedTests.length > 0) {
                md += `## 🚨 Test Failures Details\n\n`;
                for (const test of failedTests) {
                    md += `### ❌ ${test.title}\n`;
                    md += `- **File**: \\\`${test.file}\\\`\n`;
                    md += `- **Duration**: ${this.formatDuration(test.duration)}\n`;
                    if (test.error) {
                        md += `- **Error Message**:\n  \\\`\\\`\\\`text\n  ${test.error.trim().replace(/\n/g, '\n  ')}\n  \\\`\\\`\\\`\n`;
                    }
                    if (test.errorStack) {
                        md += `- **Stack Trace**:\n  \\\`\\\`\\\`text\n  ${test.errorStack.trim().replace(/\n/g, '\n  ')}\n  \\\`\\\`\\\`\n`;
                    }
                    md += `\n---\n\n`;
                }
            }
            ...
            // Write to tta-report/README.md
            const readmePath = path.join(reportDir, 'README.md');
            fs.writeFileSync(readmePath, md);

            // Also write to the root of the project for easy visibility
            fs.writeFileSync('README.md', md);
            console.log(`📝 README report generated successfully in: ${readmePath} and project root README.md`);
        } catch (error) {
            console.error('❌ Failed to generate README report:', error);
        }
    }
