...
test('Basic Web Test - Verify Page Title', async ({ page }) => {
    ...
    await page.getByText("Pytest", { exact: true }).click();//exact match is required otherwise it will throw error.
    await page.getByText("JUnit", { exact: true }).click();
    await page.keyboard.press("Escape");//Used to close the dropdown
